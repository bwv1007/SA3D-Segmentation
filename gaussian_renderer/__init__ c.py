...
_COB_RASTERIZER_AVAILABLE = False
...
try:
    ...
    cob_repo_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)),
        "submodules",
        "diff-gaussian-rasterization",
    )
    if os.path.isdir(cob_repo_path) and cob_repo_path not in sys.path:
        sys.path.append(cob_repo_path)
    try:
        from cobgs_diff_gaussian_rasterization import (  # type: ignore
            GaussianRasterizationSettings as COB_GaussianRasterizationSettings,
            GaussianRasterizer as COB_GaussianRasterizer,
...
def render(
    ...
    if _COB_RASTERIZER_AVAILABLE:
        ...
        print("[render] using COB rasterizer")
        if include_mask:
            mask_precomp = mask
            ...
            mask_signals = torch.zeros((1,), dtype=opacity.dtype, device=opacity.device)
