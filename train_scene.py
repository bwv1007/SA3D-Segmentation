...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        image, mask, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["mask"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        # 如果有-s参数（如 data/room0），则取最后一段作为输出文件夹名
        if hasattr(args, 'source_path') and args.source_path:
            folder_name = os.path.basename(args.source_path.rstrip('/\\'))
            args.model_path = os.path.join("./output/", folder_name)
        elif os.getenv('OAR_JOB_ID'):
